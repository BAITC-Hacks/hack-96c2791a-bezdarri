...
class TaskForgeTests(unittest.TestCase):
    ...
    def test_catalog_industry_filters_keep_legacy_and_draft_tasks(self):
        ...
        self.assertEqual(app.selectbox(key="catalog_industry").options, ["All", "Other", "Retail"])
        self.assertTrue(any('class="tf-badge">Other</span>' in item.value for item in app.markdown))
    ...
    def test_multiple_teams_can_submit_and_be_reviewed_independently(self):
        ...
        for team in ("First Test Team", "Second Test Team", "Third Test Team"):
            app.text_input[0].set_value(team)
            app.text_input[1].set_value("Two weeks")
            app.text_input[2].set_value("https://example.com/demo")
            app.text_area[0].set_value("Build and test a prototype")
            app.text_area[1].set_value("Interview users, build, collect feedback")
            app.button[0].click().run()
            self.assertFalse(app.exception)
            self.assertTrue(app.success)
        proposals = storage.load_records("proposals")
